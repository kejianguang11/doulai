...
class TypefaceCompatBaseImpl {
    ...
    @Nullable
    FontResourcesParserCompat.FontFamilyFilesResourceEntry a(Typeface typeface) {
        long uniqueKey = getUniqueKey(typeface);
        if (uniqueKey == 0) {
            return null;
        }
    ...
    public Typeface createFromFontInfo(Context context, @Nullable CancellationSignal cancellationSignal, @NonNull FontsContractCompat.FontInfo[] fontInfoArr, int i) throws Throwable {
        ...
        try {
            inputStreamOpenInputStream = context.getContentResolver().openInputStream(a(fontInfoArr, i).getUri());
