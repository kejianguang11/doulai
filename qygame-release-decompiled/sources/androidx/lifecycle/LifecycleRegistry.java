...
public class LifecycleRegistry extends Lifecycle {
    ...
    static class ObserverWithState {
        ...
        void a(LifecycleOwner lifecycleOwner, Lifecycle.Event event) {
            ...
            this.a = LifecycleRegistry.a(this.a, targetState);
    ...
    private void backwardPass(LifecycleOwner lifecycleOwner) {
        ...
        while (itDescendingIterator.hasNext() && !this.mNewEventOccurred) {
            ...
            while (value.a.compareTo(this.mState) > 0 && !this.mNewEventOccurred && this.mObserverMap.contains(next.getKey())) {
                Lifecycle.Event eventDownFrom = Lifecycle.Event.downFrom(value.a);
                if (eventDownFrom == null) {
                    throw new IllegalStateException("no event down from " + value.a);
                }
                pushParentState(eventDownFrom.getTargetState());
                value.a(lifecycleOwner, eventDownFrom);
                popParentState();
            }
        }
    }
    ...
    private Lifecycle.State calculateTargetState(LifecycleObserver lifecycleObserver) {
        ...
        return a(a(this.mState, entryCeil != null ? entryCeil.getValue().a : null), this.mParentStates.isEmpty() ? null : this.mParentStates.get(this.mParentStates.size() - 1));
    }
    ...
    private void forwardPass(LifecycleOwner lifecycleOwner) {
        ...
        while (iteratorWithAdditions.hasNext() && !this.mNewEventOccurred) {
            ...
            while (observerWithState.a.compareTo(this.mState) < 0 && !this.mNewEventOccurred && this.mObserverMap.contains((LifecycleObserver) next.getKey())) {
                pushParentState(observerWithState.a);
                Lifecycle.Event eventUpFrom = Lifecycle.Event.upFrom(observerWithState.a);
                if (eventUpFrom == null) {
                    throw new IllegalStateException("no event up from " + observerWithState.a);
                }
                observerWithState.a(lifecycleOwner, eventUpFrom);
                popParentState();
            }
        }
    }
    ...
    private void sync() {
        ...
        while (!isSynced()) {
            ...
            if (!this.mNewEventOccurred && entryNewest != null && this.mState.compareTo(entryNewest.getValue().a) > 0) {
    ...
    public void addObserver(@NonNull LifecycleObserver lifecycleObserver) {
        ...
        if (this.mObserverMap.putIfAbsent(lifecycleObserver, observerWithState) == null && (lifecycleOwner = this.mLifecycleOwner.get()) != null) {
            ...
            while (observerWithState.a.compareTo(stateCalculateTargetState) < 0 && this.mObserverMap.contains(lifecycleObserver)) {
                pushParentState(observerWithState.a);
                Lifecycle.Event eventUpFrom = Lifecycle.Event.upFrom(observerWithState.a);
                if (eventUpFrom == null) {
                    throw new IllegalStateException("no event up from " + observerWithState.a);
